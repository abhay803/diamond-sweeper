...
document.getElementById('user-controls').onclick = function (event) {
    var target = event.target;
    if (target.className === 'button -salmon new-game') {
        Game.init(container);
    }

    if (target.className === 'button -salmon new-game hints') {
        Game.init(container, true);
    }

    if (target.className === 'button -salmon save-game') {
        Game.saveGame();
    }

    if (target.className === 'button -salmon load-game') {
